...
export function composeResultMessageHandlers<StateA, StateB, StateC, ErrorA, ErrorB>(
    firstHandler: MessageHandler<StateA, fallible.Result<StateB, ErrorA>>,
    secondHandler: MessageHandler<StateB, fallible.Result<StateC, ErrorA | ErrorB>>
): MessageHandler<StateA, fallible.Result<StateC, ErrorA | ErrorB>> {
    return async (message, state, sockets) => {
        const firstResult = await firstHandler(message, state, sockets)
        if (!firstResult.state.ok) {
            return firstResult as MessageHandlerResult<fallible.Error<ErrorA>>
        }
...
function composeCleanupResponse<T>(
    state: T,
    cleanups: ReadonlyArray<Cleanup | undefined>
): MessageHandlerResult<T> {
...
export class ResultMessageHandlerComposer<ExistingState, NewState, Error>
    ...
    intoResultHandler<State, ErrorB>(
        other: MessageHandler<NewState, fallible.Result<State, Error | ErrorB>>
