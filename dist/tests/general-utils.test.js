...
import { cookieHeader, signedCookieHeader, parseMessageCookie, getMessageHeader, getMessageIP, getMessageMethod, getMessageURL, parseMessageContentLength, parseMessageContentType, parseURLHash, parseURLQueryString, parseSignedMessageCookie, parseURLPath, parseCookieHeader, parseContentTypeHeader, parseContentLengthHeader, parseAuthorizationHeaderBearer, parseMessageAuthorizationHeaderBearer, messageIsWebSocketRequest, parseJSONString, signatureCookieName } from '../src/general-utils.js';
...
describe('parseSignedMessageCookie', () => {
    test('returns ValueCookieMissing when cookie header missing', () => {
        const result = parseSignedMessageCookie({ headers: {} }, 'test', new Keygrip(['test key']));
        expect(result).toEqual(error('ValueCookieMissing'));
    });
    test.each([
        'test',
        'test=value',
        ...
        const result = parseSignedMessageCookie({ headers: { cookie: header } }, 'test4', new Keygrip(['test key']));
        expect(result).toEqual(error('ValueCookieMissing'));
    });
    test.each([
        'test=value',
        ...
        const result = parseSignedMessageCookie({ headers: { cookie: header } }, 'test', new Keygrip(['test key']));
        expect(result).toEqual(error('SignatureCookieMissing'));
    });
    test.each([
        ...
        const result = parseSignedMessageCookie({ headers: { cookie: header } }, name, new Keygrip(['invalid key']));
        expect(result).toEqual(error('SignatureInvalid'));
    });
    test.each([
        ...
        const result = parseSignedMessageCookie({ headers: { cookie: header } }, name, new Keygrip([key]));
...
describe('parseURLPath', () => {
    ...
    test.each([
        ...base,
        ...base.map(([url, path]) => [url + '?aaa=bbb', path]),
        ...base.map(([url, path]) => [url + '#aaa', path]),
        ...base.map(([url, path]) => [url + '?aaa=bbb#aaa', path])
...
describe('parseContentTypeHeader', () => {
    ...
    test.each([
        ...
        const result = parseContentTypeHeader(header);
        expect(result === null || result === void 0 ? void 0 : result.type).toBe(contentType.type);
        expect(result === null || result === void 0 ? void 0 : result.characterSet).toBe(contentType.characterSet);
    });
});
